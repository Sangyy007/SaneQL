...
int main() {
    // Create an ASTContainer instance
    saneql::ASTContainer astContainer;

    // Create AST nodes using the ASTContainer
    saneql::ASTBase* node1 = astContainer.allocateAST("Node 1", 1, {});
    saneql::ASTBase* node2 = astContainer.allocateAST("Node 2", 2, {});
    saneql::ASTBase* node3 = astContainer.allocateAST("Node 3", 3, {});

    // Create a list with node1 as head
    saneql::ASTBase* listHead = astContainer.createList("List Head", 4, node1);

    // Append node2 and node3 to the list
    saneql::ASTBase* appendedList = astContainer.appendList("Appended List", 5, listHead, node2);
    appendedList = astContainer.appendList("Appended List", 5, appendedList, node3);

    // Print out the content of the AST nodes
    cout << "Node 1: " << node1->getTokenValue(0) << std::endl;
    std::cout << "Node 2: " << node2->getTokenValue(0) << std::endl;
    std::cout << "Node 3: " << node3->getTokenValue(0) << std::endl;

    // Print out the content of the list
    std::cout << "List Contents: ";
    saneql::ListTemplate* list = static_cast<saneql::ListTemplate*>(appendedList);
    saneql::ASTBase* currentNode = list->head;
    while (currentNode) {
        cout << currentNode->getTokenValue(0) << " ";
        if (currentNode->descriptor == 3) {
            // Reached the end of the list
            break;
        }
        currentNode = static_cast<saneql::ListTemplate*>(currentNode)->tail;
    }
    cout << endl;
